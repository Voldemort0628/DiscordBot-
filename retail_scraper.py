...
class RetailScraper:
    ...
    def scrape_bestbuy(self, keyword: str) -> List[RetailScrapeResult]:
        ...
        try:
            ...
            if downloaded:
                soup = BeautifulSoup(downloaded, 'lxml')
                products = soup.find_all('div', {'class': 'shop-sku-list-item'})
                
                for product in products:
                    try:
                        title = product.find('h4', {'class': 'sku-title'}).text.strip()
                        price = self._clean_price(product.find('div', {'class': 'price-block'}).text)
                        product_url = 'https://www.bestbuy.com' + product.find('a')['href']
                        image_url = product.find('img')['src']
